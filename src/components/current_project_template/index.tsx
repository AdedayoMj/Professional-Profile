...
interface IHonourProps {
    sectionTitle: string;
    currentArray?: ICurrent[];
}

const CurrentTemplate: React.FunctionComponent<IHonourProps> = (props) => {
    const [deleting, setDeleting] = useState<boolean>(false);
    const [modal, setModal] = useState<boolean>(false);
    const [error, setError] = useState<string>('');

    const { currentArray, sectionTitle, children } = props;
    ...
    const deleteBlog = async (_objectID: any) => {
        setDeleting(true);
        let currentID = _objectID;
        try {
            const response = await axios({
                method: 'DELETE',
                url: `${config.server.url}api/current/${currentID}`
            });
    ...
    return (
        <section style={{ marginBottom: '3rem' }}>
            ...
            <hr />
            {currentArray?.map(function (item, index) {
                return (
                    <ul key={index}>
                        <Modal isOpen={modal}>
                        ...
                        <li>
                            {item.title} <span style={{ color: 'grey' }}>({item.attribute})</span>
                            {user._id === (item.author as IUser)._id && (
                                <Container fluid className="p-0">
                                    <Button color="info" className="mr-2" tag={Link} to={`/editCurrentProject/${item._id}`}>
